...
def bfs(atoms):
    ...
    for index, atom in enumerate(atoms):
        ...
        q.append((index, x, y, dir, energy, level))
    while not q:
        index, x, y, dir, energy, level = q.pop(0)
        ...
        if board[x][y] != 0:
            ...
            if index != ori_star[0]:
                result += energy
                result += ori_star[2]
                q = [value[0] for value in q if value[0] == index or value[0] == ori_star[0]]

        if dir == 0:
            # print("위")
            x = x - 1
            board[x][y] = (index, energy, level)
        elif dir == 1:
            # print("아래")
            x = x + 1
            board[x][y] = (index, energy, level)
...
for tc in range(1, T + 1):
    ...
    #         check_list = []
    #         # print("위")
    #         # print([atom for atom in atoms if atom[1] > y])
    #         for atom in atoms:
    #             if atom[1] > y:
    #                 check_list.append(atom)
    #         # print(list(filter(lambda star: star[1] > y, atoms)))
    #
    #     elif dir == 1:
    #         # print("아래")
    #         # 지금보다 아래 있는지
    ...
    #         # print([atom for atom in atoms if atom[1] > y])
    #
    #         # print(list(filter(lambda star: star[1] < x, atoms)))
    #
    #     elif dir == 3:
    #         # print("오른쪽")
    #         # 지금보다 오른쪽에 있는 행성이 있는지
    #         for atom in atoms:
    #             if atom[1] < x:
    #                 check_list.append(atom)
    #         # print([atom for atom in atoms if atom[1] > y])
